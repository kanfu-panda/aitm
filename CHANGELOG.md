...
- 修复终端偶尔一打开就是空白、看不到提示符（重启恢复多个标签时最常见；能正常输入，只是第一行提示符丢了）。shell 启动时打印的输出可能赶在界面开始接收之前发出而被丢掉，现在打开终端前先开始接收
- 修复在 tmux 面板新建会话后，键盘焦点停在面板的「新建」按钮上、新终端需要先点一下才能输入。现在对话框关闭后焦点直接交给新终端
- 修复分屏后重启 aitm，每个分屏都选中第一个标签、焦点总回到第一个分屏（例如 tmux 标签接回了，却停在旁边的普通标签上）。现在还原退出前每个分屏各自选中的标签和焦点所在的分屏
- 修复新建标签的标题一直显示「新标签」，要在里面 cd 过一次才变成目录名（macOS 默认的 zsh）
- 修复没有配置 AI 模型时，AI 侧栏顶部一直显示「加载中…」
- 修复标签右键「关闭其他 / 关闭右侧 / 关闭全部」会直接关掉正在跑命令的标签。现在与单个关闭一样先确认，列出哪些标签还有命令在运行；接着 tmux 的标签不算，关掉只是断开连接
- 修复一个分屏里标签多到放不下时，超出的标签和「+」按钮被裁掉、点不到。现在标签栏可以横向滚动（鼠标滚轮也行），「+」始终可见，切到或新建的标签会自动滚进可视区
- 修复在 tmux 面板改会话名后，正接着它的标签仍显示旧名字。手动改过标题的标签不受影响
- 修复在标签里手敲 `tmux attach` / `tmux new` 接入的会话没有 tmux 图标、重启也不会自动接回。现在打开 tmux 面板或退出 aitm 时会识别出来，之后与从面板接入的标签一样
- 修复退出确认写着「所有终端会话将丢失」，而接着的 tmux 会话其实会在后台继续运行。有 tmux 标签时改为如实说明
- 修复编辑器保存文件时若中途崩溃或磁盘写满，原文件可能被清空。现在先写临时文件再整体替换，保留原文件权限和符号链接
- 修复特殊环境下保存 AI Provider 设置可能导致整个应用崩溃（网络客户端创建失败时）。现在提示配置错误
- 修复终端内部状态在极端情况下出错一次后，后续每次访问都跟着崩溃
- 修复 AI 侧栏切换会话时可能残留事件监听（每次切换多留几个，长时间使用会慢慢累积）

### 安全

- 从旧版本升级的安装，`~/.aitm` 下的配置和数据文件（含 API key）权限可能仍是同组可读。现在每次启动时自动收紧为仅本人可读写
- 主窗口启用内容安全策略（CSP）：只加载应用自身的脚本，只能与本地后端通信，作为纵深防御

### 改进

- AI 对话的消息和用量改为在后台线程写入数据库，磁盘慢时不再拖慢对话
- 移除尚未实现的 `aitm doctor` 命令入口
- tmux 会话标签在标签栏里与普通标签区分开：标题前显示与 tmux 面板同款的天蓝色图标，悬停提示「tmux 会话：<名字>」；标题只显示会话名，不再加「tmux: 」前缀。重启接回时标题取会话当前的名字
